...
proton_transfer1=['A','C','E','a','c','e']
proton_transfer2=['B','D','F','b','d','f']
proton_electron_transfer=['1A','2C','3B','4D','2B','2b','1a']
two_proton_electron_transfer=['1A4D','1a-4d']
...
def Potentials_from_E(reaction,energies_ox_s,energies_red_s):
    ...
    H2_S_SMD=-1.17697911438        #Electronic energy (Ha) of H2 optimized in water at B3LYP/6-311+G(d,p)/SMD level of theory
    DE_redox_S=[]
    E_calc=[]
    if reaction in electron_transfer:
        ...
        while i < len(energies_ox_s):
            DE_redox_S.append((energies_red_s[i]-energies_ox_s[i]-H2_S_SMD)*2625.49962)
            E_calc.append((DE_redox_S[i]/(-n*F)))
            i=i+1
...
def Potentials_from_G(reaction,gibbs_ox_s,gibbs_red_s):
    ...
    H2_S_SMD_G=-1.178426           #Electronic energy (Ha) of H2 optimized in water at B3LYP/6-311+G(d,p)/SMD level of theory
    DG_redox_S=[]
    E_calc_G=[]
    if reaction in electron_transfer:
        n=1.0     #number of e- exchanged
        ...
        while i < len(gibbs_ox_s):
            DG_redox_S.append((gibbs_red_s[i]-gibbs_ox_s[i]-(H2_S_SMD_G/2))*2625.49962)
        ...
        while i < len(gibbs_ox_s):
            DG_redox_S.append((gibbs_red_s[i]-gibbs_ox_s[i]-H2_S_SMD_G)*2625.49962)
            E_calc_G.append((DG_redox_S[i]/(-n*F)))
            i=i+1
    return [DG_redox_S, E_calc_G]

def pka_from_E_ref(reaction,energies_prot_s,energies_deprot_s):
    E_HRef_solution_SMD = -345.916630593  #Electronic energy (Ha) of acac (HRef+) opt in water at B3LYP/6-311+G(d,p)/SMD
    E_Ref_solution_SMD = -345.447102223   #Electronic energy (Ha) of acac- (Ref) opt in water at B3LYP/6-311+G(d,p)/SMD
    G_H = -0.4286787                      #Gibbs free energy (Ha) -> (-269 kcal/mol)
    R =0.008314472                        #Constant of gases (kJ mol-1 k-1)
    T = 298.15                            #Temperature (K)
    pka1_HRef = 8.99                      #pKa1 experimental of acac --> acac- + H+
    DE_simple=[] 
    pka_calc_elec_simple=[]
    DE_solution=[]                        #Energy diference of the reaction including the reference system
    pka_calc_elec_ref=[]
    if reaction in proton_transfer1 or proton_transfer2:
        i=0
        print('pka1 calculated using electronic energies + reference for one proton transfer')
        while i < len(energies_prot_s):
            DE_simple.append((G_H + energies_deprot_s[i] - energies_prot_s[i])*2625.49962)
            pka_calc_elec_simple.append((DE_simple[i]/(R*T*2.303)))
            DE_solution.append((energies_deprot_s[i] + E_HRef_solution_SMD - energies_prot_s[i] - E_Ref_solution_SMD)*2625.49962)
            pka_calc_elec_ref.append((DE_solution[i]/(R*T*2.303))+pka1_HRef)
            i=i+1
    return [DE_simple, pka_calc_elec_simple, DE_solution, pka_calc_elec_ref]

def pka_from_G_ref(reaction,gibbs_prot_s,gibbs_deprot_s):
    G_HRef_solution_SMD = -345.828427  #Gibbs free energy (Ha) of acac (HRef+) optimized in water at B3LYP/6-311+G(d,p)/SMD
    G_Ref_solution_SMD = -345.370812   #Gibbs free energy (Ha) of acac- (Ref) optimized in water at B3LYP/6-31G(d)/SMD
    G_H = -0.4286787                   #Gibbs free energy (Ha) -> (-269 kcal/mol)
    R =0.008314472                     #Constant of gases (kJ mol-1 k-1)
    T = 298.15                         #Temperature (K)
    pka1_HRef = 8.99                   #pKa1 experimental of acac --> acac- + H+
    DG_simple=[]
    pka_calc_gibbs_simple=[]
    DG_solution=[]
    pka_calc_gibbs_ref=[]
    if reaction in proton_transfer1 or proton_transfer2:
        i=0
        print('pka1 calculated using Gibbs energies + reference for one proton transfer')
        while i < len(gibbs_prot_s):
            DG_simple.append((G_H + gibbs_deprot_s[i] - gibbs_prot_s[i])*2625.49962)
            pka_calc_gibbs_simple.append((DG_simple[i]/(R*T*2.303)))
            DG_solution.append((gibbs_deprot_s[i] + G_HRef_solution_SMD - gibbs_prot_s[i] - G_Ref_solution_SMD)*2625.49962)
            pka_calc_gibbs_ref.append((DG_solution[i]/(R*T*2.303))+pka1_HRef)
            i=i+1
    return [DG_simple, pka_calc_gibbs_simple, DG_solution, pka_calc_gibbs_ref]

def pka_from_E_simple(reaction,energies_prot_s,energies_deprot_s):
    R =0.008314472                     #Constant of gases (kJ mol-1 k-1)
    T = 298.15                         #Temperature (K)
    G_H = -0.4286787                   #Gibbs free energy (Ha) -> (-269 kcal/mol)
    DE_solution=[]
    ...
    if reaction in proton_transfer1 or proton_transfer2:
        ...
        while i < len(energies_prot_s):
            DE_solution.append((G_H + energies_deprot_s[i] - energies_prot_s[i])*2625.49962)
            pka_calc_elec.append((DE_solution[i]/(R*T*2.303)))
            i=i+1
    return [DE_solution, pka_calc_elec]

def pka_from_G_simple(reaction,gibbs_prot_s,gibbs_deprot_s):
    R =0.008314472                     #Constant of gases (kJ mol-1 k-1)
    T = 298.15                         #Temperature (K)
    G_H = -0.4286787                   #Gibbs free energy (Ha) -> (-269 kcal/mol)
    DG_solution=[]
    ...
    if reaction in proton_transfer1 or proton_transfer2:
        ...
        while i < len(gibbs_prot_s):
            DG_solution.append((G_H + gibbs_deprot_s[i] - gibbs_prot_s[i])*2625.49962)
            pka_calc_gibbs.append((DG_solution[i]/(R*T*2.303)))
            i=i+1
    return [DG_solution, pka_calc_gibbs]
