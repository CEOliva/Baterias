...
reaction_redox='3'       #Reaction to calculate according to PCET scheme
...
if solv_model == 'PCM':
    if electronic_energy == 'Yes':
        E_fit_elec  = [((0.592726*x) - 0.008142) for x in E_calc_elec]  #r2=0.997014
    if gibbs_energy == 'Yes':
        E_fit_gibbs = [((0.601024*x) - 0.051819) for x in E_calc_gibbs] #r2=0.993567
if solv_model == 'SMD':
    if electronic_energy == 'Yes':
        E_fit_elec  = [((0.623773*x) + 0.174499) for x in E_calc_elec]  #r2=0.997780
    if gibbs_energy == 'Yes':
        E_fit_gibbs = [((0.609969*x) + 0.073387) for x in E_calc_gibbs] #r2=0.992981
...
if solv_model == 'PCM':
    if electronic_energy == 'Yes':
        pka_fit_elec      = [((0.615043*x) + 2.232718) for x in pka_calc_elec]        #R2=0.942078 (rxnA)
    if gibbs_energy == 'Yes':
        pka_fit_gibbs     = [((0.565293*x) + 2.501698) for x in pka_calc_gibbs]       #R2=0.933843 (rxnA)

if solv_model == 'SMD':
    if electronic_energy == 'Yes':
        pka_fit_elec      = [((0.704968*x) + 1.710281) for x in pka_calc_elec]        #R2=966721 (rxnA)
        pka_fit_DE_simple = [((0.516652*x) - 146.072130) for x in DE_Solution_simple] #R2=0.966721 (rxnA)
    if gibbs_energy == 'Yes':
        pka_fit_gibbs     = [((0.641845*x) + 1.906783) for x in pka_calc_gibbs]       #R2=0.976332 (rxnA)
        pka_fit_DG_simple = [((0.470390*x) - 128.453421) for x in DG_Solution_simple] #R2=0.976332 (rxnA)
...
os.system('cxcalc Coord_opt/DFT_B3LYP_6-31Gd_Opt_SMD_HBpy+/smiles.smi pka -a 3 -b 3 > Info_files/chemaxon_pkas.csv')

#Reading pkas with chemaxon and DFT to calculate solubilities
pka_chemaxon = pd.read_csv('Info_files/chemaxon_pkas.csv',delimiter='\t')
ph_chemaxon  = pka_chemaxon['bpKa1']-1
ph_chemaxon  = ph_chemaxon.rename('ph_chemaxon')
pka_dft      = pd.DataFrame(pka_fit_gibbs)
ph_dft       = pka_dft[0] - 1
ph_dft       = ph_dft.rename('ph_dft')
smiles       = pd.read_csv('Coord_opt/DFT_B3LYP_6-31Gd_Opt_SMD_HBpy+/smiles.smi',header=None,sep='\t',usecols=[0],names=['smiles'])

#Solubily calculation with chemaxon using chemaxon and dft pH values
...
#Read de functional groups and solubility files
df_func_groups = pd.read_csv('subs.csv',header=None,names=['R1','R2','R3','R4','R5','R6','R7','R8'])
df_func_groups['core'] = 'C1([H])=C([H])C([H])=C([H])C(=N1)C1=C([H])C([H])=C([H])C([H])=N1'
df_func_groups['type'] = 'Bpy'
